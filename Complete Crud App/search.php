...
<div class="container-fluid bg-secondary p-3">
    <h1 class="text-center text-light" style="padding-bottom:10px;">STUDENT'S RECORD</h1>    
...
$sql="SELECT * FROM users WHERE name LIKE '$search%' OR id = '$search'";
...
<tbody>
    <?php while($data=mysqli_fetch_assoc($ans)){ ?>
            <tr>
                ...
                <td><a href="update.php?id=<?php echo $data['id'] ?>"><button class="btn-success p-1">Update</button></a></td>
                <td><a href="inline_del.php?id=<?php echo $data['id'] ?>"><button class="btn-danger p-1">Delete</button></a></td>    
...
<?php }
    ...
    echo '<h1 class="text-center">Could not found any Record !</h1>';
}
